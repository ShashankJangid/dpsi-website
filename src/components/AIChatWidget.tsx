...
// Advanced Knowledge Knowledge Base for DPS Indirapuram
const KNOWLEDGE_BASE: { keywords: string[]; topic: string; answer: string }[] = [
  {
    ...
    answer: " Admissions 2026-27 are open for Pre-Nursery to Class IX & XI. You can register online via the official SchoolsOS Portal (https://dpsindp.schoolforschools.ai/login). Required documents include Birth Certificate, Transfer Certificate, last report card, and passport photos. For help, contact +91-0120-4660000."
  },
  {
...
function getAIAnswer(query: string): string {
  ...
  return "DPS Indirapuram (Est. 2003) is a premier CBSE institution offering world-class academics, AI & Robotics Lab, 100% CBSE pass results (Top 99.4%), and 50+ AC GPS buses. For admissions or direct queries, please call +91-0120-4660000 or visit dpsindp.schoolforschools.ai/login.";
}
...
export default function AIChatWidget() {
  ...
  const handleSend = (userQuery: string) => {
    ...
    setTimeout(() => {
      ...
      setMessages((prev) => [...prev, { role: "assistant", text: reply, timestamp: timeStr }]);
      setIsTyping(false);
    }, 450);
  };
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-[330px] sm:w-[380px] h-[520px] bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border border-slate-200/80 dark:border-slate-800 rounded-[28px] shadow-2xl shadow-slate-900/20 flex flex-col overflow-hidden mb-3 text-slate-900 dark:text-slate-100 relative"
          >
            {/* Ethereal Smooth Ambient Aura Gradient Orbs (Inspired by User Screenshot) */}
            <div className="absolute -top-12 -right-12 w-48 h-48 rounded-full bg-cyan-400/25 dark:bg-teal-500/20 blur-3xl pointer-events-auto pointer-events-none animate-pulse" />
            ...
            {/* Header */}
            <div className="relative z-10 p-4 bg-gradient-to-r from-slate-900 via-emerald-950 to-slate-900 text-white flex items-center justify-between shadow-md">
            ...
            {/* Quick Prompt Chips */}
            <div className="relative z-10 px-3 py-2 bg-slate-50/80 dark:bg-slate-950/60 border-b border-slate-200/60 dark:border-slate-800/80 flex gap-1.5 overflow-x-auto text-[11px] no-scrollbar">
              {[
                ...
                <button
                  ...
                  className="px-3 py-1 rounded-full bg-white dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-medium whitespace-nowrap shrink-0 transition-all flex items-center gap-1.5 shadow-2xs hover:border-emerald-400"
                >
                  {chip.icon}
            ...
            {/* Messages Body */}
            <div className="relative z-10 flex-1 p-4 overflow-y-auto space-y-3.5 text-xs leading-relaxed">
              {messages.map((m, i) => (
                <div
                  key={i}
            ...
            <form
              ...
              className="relative z-10 p-3 bg-white/90 dark:bg-slate-950/90 border-t border-slate-200/70 dark:border-slate-800 flex items-center gap-2"
            >
              <input
                type="text"
                value={input}
